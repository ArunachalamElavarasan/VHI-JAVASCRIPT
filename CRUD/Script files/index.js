...
//pattern
const emailPattern =
  /^([A-Za-z0-9\.-]+)@([A-Za-z0-9\.-]+).([a-z]{2,15}).([a-z]{2,10}?)$/;
...
const insertUserDetail = (inserType, dataIndex) => {
  ...
  if (isFormValid()) {
    ...
    let dataCollection = inserType == "create" ? {} : detailsStorage[dataIndex];
    ...
    for (let index = beginValue; index < formLen; index++) {
      ...
      dataCollection[formData[index][beginValue]] =
        formData[index][beginValue] == "uploadBtn" ? imageSource : val;
    }
...
const readUserDetail = (dataIndex) => {
  resetForm();
  let updateDateItem = detailsStorage[dataIndex];
  ...
  document.querySelector(
    `input[value="${updateDateItem.gender}"]`
  ).checked = true;
...
const deleteUserDetail = (dataIndex) => {
  if (confirm(DELETE_CONFIRM)) {
    detailsStorage.splice(dataIndex, 1);
    localStorage.setItem("userData", JSON.stringify(detailsStorage));
    while (tableContainer.hasChildNodes()) {
      tableContainer.removeChild(tableContainer.lastChild);
    }
    detailsStorage.forEach((dataItem) =>
      tableContainer.appendChild(createTable(dataItem))
    );
  }
