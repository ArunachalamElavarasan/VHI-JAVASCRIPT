...
const insertUserDetail = (inserType, dataIndex) => {
  ...
  if (isFormValid()) {
    ...
    for (let index = 0; index < formLen; index++) {
      ...
      dataCollection[formData[index][0]] =
        formData[index][0] == "uploadBtn" ? imageSource : val;
    }
    if (inserType == "update") imageSource = imageContainer.src;
...
const readUserDetail = (dataIndex) => {
  ...
  genderCollection.forEach(
    (inputField) =>
      (inputField.checked =
        inputField.value == updateDateItem.gender ? true : false)
  );
...
const noValueCheck = () => {
  inputCollection.forEach((inputField) => {
    if (inputField.type != "radio" && inputField.type != "checkbox") {
      ...
      if (inputField.id == "uploadBtn" && imageContainer.src == "")
        helperText.innerHTML = NO_VALUE_ERR;
      if (
